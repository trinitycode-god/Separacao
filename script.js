...
// =====================================================================
const CONFIG = {
  APPS_SCRIPT_URL: "https://script.google.com/macros/s/AKfycbzQ2DxMndKXxwx5mJ8JFAMNZwYT9LTnlW8WKrbJYIh7_tukv8B67-tOb29cF4Tz_71M/exec"
};
  ...
  list.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <div class="item-card__meta">
        ...
        <div class="meta-block"><b>Lote</b><span>${item.lote || "—"}</span></div>
    ...
    if (done){
      footer.innerHTML = `<div class="confirmed-tag">✅ SEPARADO<small>${item.usuario || ""} · ${item.data_separacao ? formatDateTime(item.data_separacao) : ""}</small></div>`;
    } else {
      const btn = document.createElement("button");
      btn.className = "btn btn-primary btn-block";
      btn.textContent = "✅ CONFIRMAR SEPARAÇÃO";
      btn.addEventListener("click", () => confirmarSeparacao(item, btn));
...
async function confirmarSeparacao(item, btn){
  btn.disabled = true;
  btn.textContent = "SALVANDO...";
  try{
    await api("confirmarSeparacao", { id: item.id, usuario: state.user.nome });
    item.separado = 1;
